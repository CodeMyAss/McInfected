...
public class HelpCommand extends SubCommand {
	...
	public void execute(CommandSender sender, String[] args) throws CommandException {
		...
		{
			sender.sendMessage(messanger.getHeader("McInfected Commands " + args[1]));
			...
			if (sender instanceof Player)
			{
				...
				new FancyMessage(messanger.getMessage(false, Messages.Format__Prefix)).then("§4<< Back").tooltip("Go back a Help Page").command("/McInfected Help " + String.valueOf(Integer.parseInt(args[1]) - 1)).then("             ").then("§6Next >>").tooltip("Go to the next Help Page").command("/McInfected Help " + String.valueOf(Integer.parseInt(args[1]) + 1)).send((Player) sender);
			}
		}
		else
