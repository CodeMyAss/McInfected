...
public class KitCommand extends SubCommand {
	...
	public void execute(CommandSender sender, String[] args) throws CommandException {
		...
		if (!(sender instanceof Player))
			...
			sender.sendMessage(messanger.getMessage(true, Messages.Error__Game__Not_In_A_Game));
		
		else if (McInfected.getLobbyManager().getLobby((Player)sender).isStarted())
			sender.sendMessage(messanger.getMessage(true, Messages.Error__Game__Already_Started));
		
		else
			McInfected.getLobbyManager().getIPlayer((Player) sender).getiMenus().getKitsTeam().open();
