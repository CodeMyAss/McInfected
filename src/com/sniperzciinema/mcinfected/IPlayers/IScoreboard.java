...
public class IScoreboard implements Listener {
	...
	public void onPlayerToggleSneak(PlayerToggleSneakEvent e) {
		if (McInfected.getLobbyManager().isPlaying(e.getPlayer()))
			McInfected.getLobbyManager().getLobby(e.getPlayer()).getIPlayer(e.getPlayer()).getiScoreboard().switchBoardType();
	...
	private void showRegular() {
		...
		if (!iPlayer.getLobby().isStarted())
		{
			...
			{
				Score score;
				if (arena == iPlayer.getLobby().getPlayingIn())
		...
		else
		{
			...
			ob.getScore("§a§f" + iPlayer.getLobby().getHumans().size()).setScore(6);
			ob.getScore("§2").setScore(5);
			ob.getScore("§3").setScore(4);
			ob.getScore("§4§lZombies:").setScore(3);
			ob.getScore("§c§f" + iPlayer.getLobby().getZombies().size()).setScore(2);
