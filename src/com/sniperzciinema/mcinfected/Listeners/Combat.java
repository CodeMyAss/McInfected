...
public class Combat implements Listener {
	...
	public void onPlayerDamage(EntityDamageByEntityEvent e) {
		...
		if (e.getEntity() instanceof Player)
		{
			...
			{
				Lobby lobby = McInfected.getLobbyManager().getLobby(victim);
				...
				if ((killer instanceof Player) && lobby.isIPlayer(killer))
				{
					IPlayer iVictim = lobby.getIPlayer(victim);
					IPlayer iKiller = lobby.getIPlayer(killer);
					
					// Make sure they arn't on the same team
					if (iVictim.getTeam() != iKiller.getTeam() && McInfected.getLobbyManager().getLobby(victim).isStarted())
					{
						...
						if ((victim.getHealth() - e.getDamage()) <= 0)
						{
							...
							McInfected.getMessanger().broadcast(lobby, McInfected.getMessanger().getDeathMessage(iVictim.getTeam(), death, "<killer>", killer.getName(), "<killed>", victim.getName()));
							
							if (lobby.getHumans().isEmpty())
							{
								McInfected.getMessanger().broadcast(lobby, McInfected.getMessanger().getMessage(true, Messages.Game__Over__Zombies_Win));
	...
	public void onPlayerDie(PlayerDeathEvent e) {
		if (McInfected.getLobbyManager().isPlaying(e.getEntity()))
		{
			final Lobby lobby = McInfected.getLobbyManager().getLobby(e.getEntity());
			e.setDeathMessage("");
			e.setDroppedExp(0);
			IPlayer iVictim = lobby.getIPlayer(e.getEntity());
			...
			if (lobby.isStarted())
			{
				...
				if (iVictim.getLastDamager() != null)
				{
					...
					McInfected.getMessanger().broadcast(lobby, McInfected.getMessanger().getDeathMessage(iVictim.getTeam(), DeathType.OTHER, "<killer>", iKiller.getPlayer().getName(), "<killed>", iVictim.getPlayer().getName()));
				}
				if (lobby.getHumans().isEmpty())
				{
					McInfected.getMessanger().broadcast(lobby, McInfected.getMessanger().getMessage(true, Messages.Game__Over__Zombies_Win));
